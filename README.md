...
- 왼쪽 `경쟁사 상품 설정` 페이지에서 상품별/경쟁사별로 `검색여부`와 정확한 네이버 상품 URL을 관리합니다.
- 기본 `검색여부`는 모두 체크 상태입니다. URL이 비어 있으면 실제 페이지 요청은 하지 않습니다.
- `검색규칙`에서 상품을 새로 추가하면 경쟁사 상품 설정 표에도 자동으로 행이 추가됩니다. 새 행의 기본값은 `검색여부=True`, `상품URL=빈칸`입니다.
- 경쟁사 URL은 한 번에 몰아서 요청하지 않고 각 요청 사이 2~5초 간격을 둡니다. 실패 시 1회만 재시도하며 CAPTCHA/접근제한이 감지되면 우회하지 않고 해당 가격을 빈칸 처리합니다.
- 경쟁사 상품의 단위가 다르면 기존 `가격규칙` 페이지에서 해당 경쟁사에 `mul` 등의 환산 규칙을 추가할 수 있습니다.
